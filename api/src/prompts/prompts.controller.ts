...
export class PromptsController {
  ...
  getPromptById(@Param('id') id: string, @CurrentUser() user?: { walletAddress: string }) {
  ...
  createPrompt(
    ...
    return this.promptsService.createPrompt(createPromptDto, user.walletAddress);
  }
  ...
  purchasePrompt(
    ...
    return this.promptsService.purchasePrompt(id, purchaseDto, user.walletAddress);
  }
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - you must purchase the prompt first' })
  @ApiResponse({ status: 404, description: 'Prompt not found' })
  ratePrompt(
    @Param('id') id: string,
